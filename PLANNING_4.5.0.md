...
On-device VERIFIED Sep 14 2026 (RC4 foss debug, moto g play): Finding C confirmed end to end. A clearsigned message from a good (never-revoked) key verifies as Verified; a clearsigned message from a fully-revoked key (gpg auto-revocation cert, colon-prefix stripped, re-imported) downgrades to Invalid with a revocation reason instead of showing Verified. Findings A (Argon2 policy) and B (decompression/size caps) stay unit-covered (Argon2PolicyTest) plus effective regression from item 12 normal password/compressed decrypts passing through the guards unrejected; Finding D (SEIPDv1 error uniformity) stays unit-covered (Seipdv1TamperTest). The crafted-attack cases for A/B/D are not practically reproducible with a non-PQC sq / gpg 2.5.21 setup. NOTE: Finding B still has no dedicated decompression-bomb unit test (happy-path zlib round-trips only) - candidate follow-up.
...
ML-KEM-1024+X448 imported-decrypt (Umotas, RC8): the reported "I can import ML-KEM-1024+X448 keys but cannot decrypt the FILE" is NOT a crypto gap. The crypto core and the in-memory/text decrypt handle it end to end (CompositeImported1024DecryptTest: parse of the composite ML-DSA-87 primary green, in-memory decrypt green). Root cause is the STREAMING file path: PGPCryptoService.decryptStream -> CompositeDecryptor.recoverSessionKey hardcoded emptyList() for the raw composite-primary rings, and EncryptDecryptViewModel.decryptFileStreamingPath never gathered them, so a file (content URI) encrypted to an imported composite-PRIMARY key never reached findRawComposite/openRaw and failed with "no held composite secret key". The in-memory and file-bytes paths thread the rings, which is why item 12's 768 device check passed (non-streaming) and pasting the same message as text worked. Fix (RC8): recoverSessionKey gains a rawCompositeRings param (passed through to recoverAmong instead of emptyList); decryptStream gains compositePrimaryRings and passes it to recoverSessionKey; decryptFileStreamingPath now gathers composite rings (isCompositeSign -> loadCompositePrivateRing) exactly like the in-memory paths. Fixture from Umotas's own sq PQC build (v6, ML-DSA-87+Ed448 cert primary + ML-DSA-87 sign/auth subkeys + ML-KEM-1024+X448 encryption subkey, passphrase SEZAM): app/src/test/resources/pqc/sq-1024-sec.asc + sq-1024-msg.gpg. Regression test CompositeImported1024DecryptTest covers parse, in-memory decrypt, and the streaming path (the one that was broken). CompositeAnonymousPkeskTest / CompositeStreamDecryptTest re-run green after the signature change.
...
LibrePGP counterpart (a tester, PGPony Android 4.4.1, Pixel 8): the identical bug lived in the SEPARATE algo-8 path, CompositeLibrePGPDecryptor, which the #57 fix above never touched (that was the IETF algo-35/36 CompositeDecryptor). Reported as a multi-recipient message to all-LibrePGP-PQC recipients failing with "no held LibrePGP composite secret key", even for PGPony-generated keys; single-recipient to self worked. Same root cause: split() kept one PKESK, recover() tried one. Fixed the same way for RC6: split() and a new allPkesks() collect every algo-8 PKESK, and a new recoverAmong() tries each held key against every slot (addressed and anonymous), propagating a locked-key error and throwing NoMatchingKey only after all slots miss. Regression test: CompositeLibrePGPMultiRecipientDecryptTest (768 + 1024 LibrePGP, both recipients decrypt), mirroring CompositeMultiRecipientDecryptTest on the IETF side.

Mixed-message half (tester follow-ups, RC7): The tester narrowed it to "any multi-recipient message with a post-quantum key fails; all-traditional works." The multi-composite fix above covers a COMPOSITE-key holder, but a CLASSICAL-key holder of a mixed message (their slot classical, a co-recipient composite) still failed: the composite decryptor found no held composite key and threw, and BouncyCastle cannot parse a message that still carries a composite PKESK, so it never reached the classical slot. Fixed for RC7: PGPCryptoService.decrypt now CATCHES the composite NoMatchingKey into compositeMiss instead of failing, and in the BC branch, when a composite slot missed, CompositePkeskStripper.strip() removes the composite PKESK packets (algo 8/35/36, v3 and v6) and hands the classical PKESKs + SKESK + body to BC. Every PKESK wraps the same session key, so BC opens the classical slot and decrypts the body. If stripping leaves no classical/SKESK slot (a pure-composite no-key message), the original composite miss is re-thrown so the informative error is preserved. Regression test: CompositeMixedRecipientDecryptTest (classical holder opens a mix with a LibrePGP and with an IETF composite co-recipient, and the composite holder still opens it). Note: this covers the in-memory decrypt path (text messages, the tester's case); the streaming/file path (decryptStream) keeps the pre-RC7 behavior and is a parallel follow-up if a mixed FILE to a classical-only holder comes up.
...
separately upstream so it is not suppressed. KeyDeduplicationExpiryGuardTest locks the guard. lukascomer re-imports
...
## 26. PQ-only (composite ML-DSA) keys cannot generate a QR code, even multipart (FIXED in RC)

Priority: medium-high. Origin: NorseHorse, on-device 4.5.0 verify batch (Sep 14 2026, RC4 foss release smoke). Sharing a PQ-only key by QR shows the "too large, use Share or Copy" message and produces no QR at all, single or multipart.
...
## 31. Recipient picker cannot tell apart keys with the same name (limbodiver, email)

Priority: medium. Origin: limbodiver (email, Sep 15 2026) - when several keys share a display name, the recipient list shows only the name and a short key code (shortFingerprint), so the rows look identical and he had to type the full email to land on the right key. Reporter is a Google Play user, so this ships in the 4.5.0 Play release, not an RC.
...
## 32. Adding a second User ID silently makes it primary on upload (limbodiver, email)

Priority: high. Origin: limbodiver (email, Sep 16 2026) - added a second email to his key without marking it primary; after uploading to a keyserver the new address showed as primary. Reporter is a Google Play user, so this ships in the 4.5.0 Play release.
